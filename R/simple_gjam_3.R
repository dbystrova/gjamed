.gjam_3 <- function(formula, xdata, ydata, modelList){
 ...
 # if(TRAITS) rownames(specByTrait) <- colnames(y)
  ...
  reductList <- .setupReduct(modelList, S, Q, n) ##########check but shoulnd't modify something
  N <- reductList$N
  r <- reductList$r
  sigma_py<-reductList$sigma_py
  alpha.DP<-reductList$alpha
  ...
  CLUST <- T   # dirichlet ??? flag  
  
  .param.fn <- .paramWrapper_3(REDUCT, inSamp, SS=length(notOther))
  sigmaerror <- .1
