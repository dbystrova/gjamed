...
.setupReduct <- function(modelList, S, Q, n){
  ...
  PYeps<-0.1
  PYtheta<-1
  N<-floor(.compute_tau_mean(PYsigma,PYtheta,PYeps) + 2*.compute_tau_var(PYsigma,PYtheta,PYeps))
...
.gjam <- function(formula, xdata, ydata, modelList){
  ...
  if(REDUCT){
    ...
    otherpar$pvec     <- .sampleP(N=N, avec=rep(1 -0.4,(N-1)),
                                  bvec=((1:(N-1))*0.4 + alpha.DP), K=otherpar$K)
...
.getPars <- function(CLUST, x, N, r, Y, B, D, Z, sigmaerror, K, pvec,
  ...
  if(CLUST){   #only until convergence
    ...
    pvec <- .sampleP(N=N, avec=rep(1 -0.4,(N-1)),
                     bvec=((1:(N-1))*0.4 + alpha.DP), K=K)
