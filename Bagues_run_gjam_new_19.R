...
train<- load_object("sample_data_train2.Rds")
test<- load_object("sample_data_test2.Rds")
...
#no Holdout
fit_19<- load_object("models_Bagues_data_OSS/fit_3.Rda")
#fit_text<- load_object("models_Bagues_data_OSS/fit_3.Rda")
...
p1  <- gjamPredict(output = fit_19, newdata = new)
...
fit2_19<- load_object("models_Bagues_data_OSS/fit2_3.Rda")
...
fit3_19<- load_object("models_Bagues_data_OSS/fit3_3.Rda")
...
fit4_19<- load_object("models_Bagues_data_OSS/fit4_3.Rda")
...
Mat<- fit4_19$chains$kgibbs[(burn+1):it,]
...
CM_DP1<- comp.psm(fit_19$chains$kgibbs[(burn+1):it,])
CM_DP2<- comp.psm(fit2_19$chains$kgibbs[(burn+1):it,])
CM_PY1<- comp.psm(fit3_19$chains$kgibbs[(burn+1):it,])
...
pk_chains2_last<- mcmc(fit2_19$chains$pk_g[burn:it,ncol(fit2$chains$pk_g)])
plot(pk_chains2_last)
GJAM2_pk_last<- mean(pk_chains2_last)

pk_chains3_last<- mcmc(fit3_19$chains$pk_g[burn:it,ncol(fit3$chains$pk_g)])
plot(pk_chains3_last)
PY1_pk_last<- mean(pk_chains3_last)

pk_chains4_last<- mcmc(fit4_19$chains$pk_g[burn:it,ncol(fit4$chains$pk_g)])
...
#plot(pk_chains1_last)
pk_chains2_last<- mcmc(fit2_19$chains$pk_g[,ncol(fit2$chains$pk_g)])
plot(pk_chains2_last)
pk_chains3_last<- mcmc(fit3_19$chains$pk_g[,ncol(fit3$chains$pk_g)])
plot(pk_chains3_last)
pk_chains4_last<- mcmc(fit4_19$chains$pk_g[,ncol(fit4$chains$pk_g)])
plot(pk_chains4_last)
